...
async function load() {
  loading.value = true
  try { hosts.value = await listHosts() } catch (err) { toast.error(extractError(err)) }
...
<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <Modal :open="showModal" :title="editing ? 'Host bearbeiten' : 'Host hinzufuegen'" @close="showModal = false">
