...
const AGENT_LABELS: Record<AuftragAgent, string> = { claude: 'Claude', codex: 'Codex', gemini: 'Gemini', auto: 'Automatisch' }
...
const STATUS_LABELS: Record<AuftragStatus, string> = { eingang: 'Eingang', geplant: 'Geplant', laeuft: 'Läuft', rueckfrage: 'Rückfrage', freigabe: 'Plan zur Freigabe', unterbrochen: 'Unterbrochen', fertig: 'Fertig', fehler: 'Fehler', abgebrochen: 'Abgebrochen' }
...
const VORSCHLAG_AGENTEN = AGENTEN.filter((agent) => agent.id !== 'auto')
...
function agentChipLabel(auftrag: Auftrag): string {
    ...
    <Transition name="panel">
      <aside v-if="panel" class="panel" :class="{ 'panel-klein': panel === 'vorschlaege' }" :aria-label="panel === 'neu' ? 'Neuer Auftrag' : panel === 'vorschlaege' ? 'Vorschläge einholen' : 'Auftragsdetails'">
        ...
        <form v-else-if="panel === 'vorschlaege'" class="formular vorschlaege-form" @submit.prevent="analyseStarten">
          ...
          <p class="projekt-hinweis">Projektliste aus flow-agent (alle Hosts) und Werkstatt.</p>
          <fieldset class="agent-auswahl vorschlaege-agenten"><legend>Agent</legend><label v-for="agent in VORSCHLAG_AGENTEN" :key="agent.id" class="agent-radio" :class="{ aktiv: vorschlaegeForm.agent === agent.id }"><input v-model="vorschlaegeForm.agent" type="radio" name="vorschlaege-agent" :value="agent.id" /><span><strong>{{ agent.titel }}</strong><small>{{ agent.text }}</small></span></label></fieldset>
        ...
        <div v-else-if="ausgewaehlt" class="detail">
          ...
          <template v-else>
            <div class="detail-chips chips"><span :class="['agent-chip', `agent-${ausgewaehlt.agent}`]" :title="ausgewaehlt.agent_auto && ausgewaehlt.agent_grund ? ausgewaehlt.agent_grund : undefined">{{ agentChipLabel(ausgewaehlt) }}</span><span class="modus-chip">{{ MODUS_LABELS[ausgewaehlt.modus] }}</span><span>{{ PROFIL_LABELS[ausgewaehlt.profil] }}</span><span>P{{ ausgewaehlt.prioritaet }}</span><span>{{ ZEIT_LABELS[ausgewaehlt.zeitfenster] }}</span></div>
            <p v-if="ausgewaehlt.agent_auto && ausgewaehlt.agent_grund" class="agent-grund mono">{{ ausgewaehlt.agent_grund }}</p>
            ...
            <section v-if="ausgewaehlt.status === 'unterbrochen'" class="detail-block unterbrochen-block"><h3>Lauf unterbrochen</h3><p class="prosa">{{ ausgewaehlt.fehler || 'Der laufende Prozess ist nicht mehr erreichbar.' }}</p><div class="aktionen"><button class="knopf" type="button" :disabled="busy" @click="fortsetzenAktion">Fortsetzen</button><button class="knopf ghost" type="button" :disabled="busy" @click="statusSetzen('eingang', 'Auftrag liegt wieder im Eingang')">Erneut in Eingang</button></div></section>
            ...
            <section v-if="ausgewaehlt.status !== 'freigabe' && ausgewaehlt.status !== 'unterbrochen' && (ausgewaehlt.ergebnis || ausgewaehlt.fehler)" class="detail-block"><h3>{{ ausgewaehlt.fehler ? 'Fehler' : 'Ergebnis' }}</h3><p class="prosa" :class="{ 'fehler-text': ausgewaehlt.fehler }">{{ ausgewaehlt.fehler ?? ausgewaehlt.ergebnis }}</p><a v-if="ausgewaehlt.diff_url" class="diff-link" :href="ausgewaehlt.diff_url" target="_blank" rel="noopener"><ExternalLink :size="14" /> Diff öffnen</a></section>
            <section v-if="ausgewaehlt.status === 'fertig' || ausgewaehlt.pruefung" class="detail-block pruefung-block"><div class="block-kopf"><h3>Prüfung im Worktree</h3><span class="pruefung-badge" :class="ausgewaehlt.pruefung_ok === true ? 'ok' : ausgewaehlt.pruefung_ok === false ? 'krit' : 'ohne'">{{ ausgewaehlt.pruefung_ok === true ? 'Prüfung ✓' : ausgewaehlt.pruefung_ok === false ? 'Prüfung ✗' : 'ohne Prüfung' }}</span></div><div v-if="ausgewaehlt.pruefung?.length" class="pruefung-liste"><details v-for="(test, index) in ausgewaehlt.pruefung" :key="`${test.befehl}-${index}`" class="pruefung-zeile"><summary><span :class="test.ok ? 'test-ok' : 'test-fehler'">{{ test.ok ? '✓' : '✗' }}</span><code>{{ test.befehl }}</code><span class="mono dim">{{ dauerText(test.dauer_s) }}</span></summary><pre>{{ test.auszug || 'Kein Auszug vorhanden.' }}</pre></details></div><p v-else class="dim ohne-pruefung">Keine Prüfbefehle gefunden.</p><button class="knopf klein ghost" type="button" :disabled="busy" @click="pruefenAktion">Erneut prüfen</button></section>
            <section v-if="ausgewaehlt.pr_url || (ausgewaehlt.status === 'fertig' && ausgewaehlt.branch)" class="detail-block pr-block"><h3>Pull Request</h3><template v-if="ausgewaehlt.pr_url"><a class="pr-link" :href="ausgewaehlt.pr_url" target="_blank" rel="noopener"><ExternalLink :size="14" /> Pull Request auf GitHub öffnen</a><span v-if="ausgewaehlt.pr_checks" class="mono pr-checks">{{ ausgewaehlt.pr_checks }}</span></template><template v-else><p class="prosa">Pusht den Branch und legt den PR an – gemergt wird nur von dir auf GitHub.</p><p v-if="ausgewaehlt.pruefung_ok === false" class="pr-warnung">Prüfung ist rot</p><button class="knopf" type="button" :disabled="busy" @click="prErstellenAktion">PR erstellen</button></template></section>
            ...
            <div class="aktionen detail-aktionen">
              ...
              <template v-else-if="ausgewaehlt.status === 'freigabe' || ausgewaehlt.status === 'unterbrochen'" />
            ...
            <div v-if="['fertig', 'fehler', 'abgebrochen'].includes(ausgewaehlt.status) && ausgewaehlt.worktree" class="aufraeumen"><button class="knopf ghost" type="button" :disabled="busy" @click="aufraeumenAktion(false)">Worktree aufräumen</button><button class="branch-loeschen" type="button" :disabled="busy" @click="aufraeumenAktion(true)">{{ branchLoeschenBestaetigen ? 'Wirklich löschen?' : '… und Branch löschen' }}</button></div>
          </template>
        </div>
      </aside>
    </Transition>
...
.kapazitaet > strong { color: var(--ok); font-size: 10px; text-transform: uppercase; letter-spacing: .05em; }.kapazitaet.angehalten { border-color: rgba(242,184,75,.55); background: rgba(242,184,75,.09); }.kapazitaet.angehalten > strong, .runner-warnung { color: var(--warn); }.pause { color: var(--warn); font-family: var(--mono); }.runner-warnung { font-family: var(--mono); font-size: 10px; }.runner-schalter { margin-left: auto; padding: 3px 8px; border: 1px solid var(--linie); border-radius: 5px; background: transparent; color: var(--text-2); font-family: var(--display); font-size: 10px; font-weight: 700; letter-spacing: .05em; text-transform: uppercase; }.runner-schalter:hover { border-color: var(--akzent); color: var(--akzent); }
...
.karten { display: flex; flex-direction: column; gap: 9px; }.auftrag { position: relative; background: var(--flaeche-2); border: 1px solid var(--linie); border-radius: 7px; padding: 11px 11px 10px; box-shadow: 0 5px 16px rgba(0,0,0,.12); cursor: pointer; transition: transform .16s ease, border-color .16s ease, opacity .16s ease; }.auftrag:hover { border-color: #42517F; transform: translateY(-1px); outline: none; }.auftrag:focus-visible { border-color: var(--akzent); transform: translateY(-1px); outline: 2px solid var(--akzent); outline-offset: 2px; }.auftrag.gezogen { opacity: .35; }.auftrag.status-laeuft { border-left: 2px solid var(--ok); }.auftrag.status-rueckfrage, .auftrag.status-freigabe { border-left: 2px solid var(--akzent); }.auftrag.status-unterbrochen { border-left: 2px solid #7185A8; }.auftrag.status-freigabe { box-shadow: inset 0 0 18px rgba(242,184,75,.035), 0 5px 16px rgba(0,0,0,.12); }.auftrag.status-fehler { border-left: 2px solid var(--krit); }.auftrag.status-abgebrochen { border-left: 2px solid var(--text-3); }
...
.status-marke.freigabe { color: var(--akzent); border-color: rgba(242,184,75,.6); background: rgba(242,184,75,.1); }.status-marke.unterbrochen { color: #9AAAC5; border-color: rgba(113,133,168,.65); background: rgba(113,133,168,.12); }.chips .modus-chip { color: #D7C8A2; border-color: rgba(242,184,75,.32); }
...
.chips .agent-chip.agent-claude { color: #F2AA6B; border-color: rgba(231,154,90,.55); background: rgba(231,154,90,.08); }.chips .agent-chip.agent-codex { color: #60D5B7; border-color: rgba(76,195,165,.55); background: rgba(76,195,165,.08); }.chips .agent-chip.agent-gemini { color: #7DB5FF; border-color: rgba(111,168,255,.55); background: rgba(111,168,255,.08); }.chips .agent-chip.agent-auto { color: var(--akzent); border-color: rgba(242,184,75,.5); background: rgba(242,184,75,.08); }
...
.qualitaet-chips { display: flex; flex-wrap: wrap; align-items: center; gap: 5px; margin-top: 8px; }.pruefung-badge, .pr-chip { display: inline-flex; align-items: center; padding: 2px 6px; border: 1px solid var(--linie); border-radius: 9px; font-family: var(--mono); font-size: 8px; line-height: 1.35; }.pruefung-badge.ok { color: var(--ok); border-color: rgba(76,195,138,.48); background: rgba(76,195,138,.07); }.pruefung-badge.krit { color: var(--krit); border-color: rgba(242,109,109,.48); background: rgba(242,109,109,.07); }.pruefung-badge.ohne { color: var(--text-3); }.pr-chip { color: var(--info); border-color: rgba(111,168,255,.48); background: rgba(111,168,255,.07); text-decoration: none; }.pr-chip:hover { border-color: var(--info); color: #A9CAFF; }
.unterbrochen-fehler { margin: 8px 0 0; color: #9AAAC5; font-size: 10px; line-height: 1.4; }
...
.unterbrochen-block { padding: 16px; border: 1px solid rgba(113,133,168,.5); border-radius: 7px; background: rgba(113,133,168,.08); }.unterbrochen-block .prosa { color: #B5C0D4; }.aufraeumen { display: flex; align-items: center; flex-wrap: wrap; gap: 9px; margin-top: 13px; padding-top: 13px; border-top: 1px solid var(--linie); }.branch-loeschen { border: 0; background: transparent; color: var(--text-3); font-size: 10px; text-decoration: underline; text-underline-offset: 2px; }.branch-loeschen:hover { color: var(--krit); }
