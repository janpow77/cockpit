// Globaler Confirm-Store (KOPIE des llm-router-Patterns).
// Jede mutierende UI-Aktion fragt VOR dem API-Call:
...
export const useConfirmStore = defineStore('confirm', () => {
  ...
  function resolve(ok: boolean) {
    if (pending.value) pending.value.resolve(ok)
    pending.value = null
    open.value = false
  }

  function confirm() { resolve(true) }
