...
function mockOverview(): Overview {
  return {
    ...
    projects: [
      ...
      { host: 'ccx23', name: 'flowinvoice', title: 'flowinvoice', sub: 'Belegprüfung mit Erkennung', containers: 8, running: 8, status: 'healthy', images: [], url: null, tunnel: false, registered: false, app_id: null, app_status: null, last_check_at: null, deploy: null },
...
export async function startDemo(): Promise<DemoStartResult> {
  ...
  const { data } = await client.post<DemoStartResult>('/overview/demo', {}, { timeout: 330_000 })
  return data
}
