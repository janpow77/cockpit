...
export type AuftragPatch = Partial<Pick<Auftrag, 'status' | 'prioritaet' | 'reihenfolge' | 'titel' | 'text' | 'agent' | 'profil' | 'zeitfenster'>>

const jetzt = Date.now()
const iso = (versatz = 0) => new Date(jetzt + versatz).toISOString()
let mockAuftraege: Auftrag[] = [
  { id: 'a-1', titel: 'Barrierefreiheit der Formulare prüfen', text: 'Prüfe die Formulare auf Tastaturbedienung und sinnvolle Labels. Dokumentiere die Befunde.', host: 'nuc', projekt: '/home/jan/projekte/regulierung', projekt_name: 'regulierung', agent: 'claude', profil: 'lesen', prioritaet: 3, zeitfenster: 'sofort', status: 'eingang', reihenfolge: 1, branch: null, worktree: null, session_id: null, gestartet: null, beendet: null, dauer_s: null, ergebnis: null, fehler: null, kosten_usd: null, tokens_in: null, tokens_out: null, turns: null, letzte_zeile: null, diff_url: null, erstellt: iso(-7_200_000), aktualisiert: iso(-7_200_000) },
  { id: 'a-2', titel: 'Lint-Warnungen im Cockpit beheben', text: 'Behebe die aktuellen Lint-Warnungen, führe Type-Check und Tests aus und committe mit sprechender Meldung.', host: 'nuc', projekt: '/home/jan/projekte/cockpit', projekt_name: 'cockpit', agent: 'codex', profil: 'bearbeiten_tests', prioritaet: 2, zeitfenster: 'nachts', status: 'geplant', reihenfolge: 1, branch: null, worktree: null, session_id: null, gestartet: null, beendet: null, dauer_s: null, ergebnis: null, fehler: null, kosten_usd: null, tokens_in: null, tokens_out: null, turns: null, letzte_zeile: null, diff_url: null, erstellt: iso(-5_400_000), aktualisiert: iso(-3_600_000) },
  { id: 'a-3', titel: 'API-Tests für Exporte ergänzen', text: 'Ergänze Tests für die Export-Endpunkte und behebe gefundene Randfälle.', host: 'ccx23', projekt: '/srv/regulierung', projekt_name: 'regulierung', agent: 'claude', profil: 'bearbeiten_tests', prioritaet: 1, zeitfenster: 'sofort', status: 'laeuft', reihenfolge: 1, branch: 'agent/export-tests', worktree: '/tmp/auftrag-a-3', session_id: 'codex-a-3', gestartet: iso(-754_000), beendet: null, dauer_s: null, ergebnis: null, fehler: null, kosten_usd: null, tokens_in: 18420, tokens_out: 2310, turns: 7, letzte_zeile: 'Running pytest tests/api/test_exports.py …', diff_url: null, erstellt: iso(-10_800_000), aktualisiert: iso(-20_000) },
  { id: 'a-4', titel: 'Entscheidung zur Migration nötig', text: 'Bereite die Migration auf das neue Schema vor.', host: 'nuc', projekt: '/home/jan/projekte/flowinvoice', projekt_name: 'flowinvoice', agent: 'gemini', profil: 'voll', prioritaet: 2, zeitfenster: 'sofort', status: 'rueckfrage', reihenfolge: 1, branch: 'agent/schema-migration', worktree: '/tmp/auftrag-a-4', session_id: 'codex-a-4', gestartet: iso(-3_000_000), beendet: null, dauer_s: 1142, ergebnis: 'Für die Spalte `beleg_typ` gibt es zwei mögliche Migrationswege. Soll sie zunächst nullable bleiben?', fehler: null, kosten_usd: 0.41, tokens_in: 41200, tokens_out: 4900, turns: 12, letzte_zeile: 'Warte auf Rückmeldung.', diff_url: null, erstellt: iso(-14_400_000), aktualisiert: iso(-1_800_000) },
  { id: 'a-5', titel: 'Dokumentation der Backup-Routine', text: 'Aktualisiere die Betriebsdokumentation.', host: 'ccx23', projekt: '/srv/cockpit', projekt_name: 'cockpit', agent: 'codex', profil: 'bearbeiten', prioritaet: 4, zeitfenster: 'nach_reset', status: 'fertig', reihenfolge: 1, branch: 'agent/backup-docs', worktree: '/tmp/auftrag-a-5', session_id: 'codex-a-5', gestartet: iso(-86_400_000), beendet: iso(-86_081_000), dauer_s: 319, ergebnis: 'Die Backup-Routine ist jetzt mit Wiederherstellungsschritten und Prüfkommandos dokumentiert.', fehler: null, kosten_usd: 0.18, tokens_in: 12300, tokens_out: 1800, turns: 5, letzte_zeile: 'Commit erstellt.', diff_url: 'https://github.com/example/cockpit/compare/agent/backup-docs', erstellt: iso(-90_000_000), aktualisiert: iso(-86_081_000) },
  { id: 'a-6', titel: 'Veralteten Import entfernen', text: 'Entferne den veralteten Import und prüfe den Build.', host: 'nuc', projekt: '/home/jan/projekte/audit-designer', projekt_name: 'audit-designer', agent: 'claude', profil: 'bearbeiten_tests', prioritaet: 3, zeitfenster: 'sofort', status: 'fehler', reihenfolge: 2, branch: 'agent/import-cleanup', worktree: '/tmp/auftrag-a-6', session_id: 'codex-a-6', gestartet: iso(-45_000_000), beendet: iso(-44_930_000), dauer_s: 70, ergebnis: null, fehler: 'Abhängigkeit konnte nicht installiert werden: Registry nicht erreichbar.', kosten_usd: 0.06, tokens_in: 3900, tokens_out: 420, turns: 2, letzte_zeile: 'npm ERR! network timeout', diff_url: null, erstellt: iso(-46_000_000), aktualisiert: iso(-44_930_000) },
]
...
const mockVorlagen: Vorlage[] = [
  { id: 'fehler-beheben', titel: 'Fehler in {projekt} beheben', profil: 'bearbeiten_tests', prioritaet: 2, text: 'Analysiere den beschriebenen Fehler, behebe die Ursache und führe die passenden Tests sowie den Linter aus. Committe mit einer sprechenden Meldung.' },
  { id: 'code-review', titel: 'Code-Review für {projekt}', profil: 'lesen', prioritaet: 3, text: 'Prüfe die aktuellen Änderungen auf Fehler, Sicherheitsrisiken und Wartbarkeit. Ändere keine Dateien und fasse die Befunde nach Priorität zusammen.' },
  { id: 'dokumentation', titel: 'Dokumentation von {projekt} aktualisieren', profil: 'bearbeiten', prioritaet: 4, text: 'Prüfe die bestehende Dokumentation auf veraltete Angaben und ergänze die fehlenden Betriebs- und Entwicklungshinweise.' },
]
...
export async function anlegen(body: AuftragAnlegen): Promise<Auftrag> {
  if (USE_MOCKS) {
    ...
    const neu: Auftrag = { id: `a-${Date.now()}`, ...body, projekt_name: projekt?.name ?? body.projekt.split('/').pop() ?? body.projekt, status: 'eingang', reihenfolge: mockAuftraege.length + 1, branch: null, worktree: null, session_id: null, gestartet: null, beendet: null, dauer_s: null, ergebnis: null, fehler: null, kosten_usd: null, tokens_in: null, tokens_out: null, turns: null, letzte_zeile: null, diff_url: null, erstellt: new Date().toISOString(), aktualisiert: new Date().toISOString() }
...
export async function vorschlaegeEinholen(body: Pick<AuftragAnlegen, 'host' | 'projekt' | 'agent'>): Promise<Auftrag> {
  if (USE_MOCKS) {
    ...
    const auftrag = await anlegen({ ...body, titel: `Vorschläge für ${projekt?.name ?? 'Projekt'} ermitteln`, text: 'Analysiere das Projekt und lege 5–10 priorisierte Vorschläge als Aufträge in den Eingang.', profil: 'lesen', prioritaet: 3, zeitfenster: 'sofort' })
