...
- **evo-x2 (`evo2`, Tailscale 100.81.4.99):** **PRIMÄRER LLM-Backend** — die Router-Routen `qwen3:14b`/`qwen3.5:*`/`sarah-qwen36*` zeigen alle hierher (ollama :11434, serviert qwen3.5:35b-fast/qwen3:14b/sarah-qwen36/qwen3.5:9b-fast/bge-m3). Deploy = systemd (evo-Repo) + **täglicher `llama-update.timer`** (Auto-Update!). Separate llama.cpp-Dienste :8080–8083 + evo-metrics :8084, webhook :9000. **NICHT in cockpit-bootstrap** → zentral unüberwacht (nur via Router-Spoke-Health). evo-gpu-llm-manager :7842 = offline/tot.
