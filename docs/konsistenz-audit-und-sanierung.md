...
- **M11c (evo-x2 ollama härten):** ollama läuft als **bare root-Prozess ohne systemd-Unit** (kein Auto-Restart) auf **Defaults** (keine `OLLAMA_NUM_PARALLEL`/`MAX_LOADED_MODELS`/`KEEP_ALIVE` gesetzt); `evo-health-monitor.service` = FAILED. Maßnahmen: (1) ollama in eine **systemd-Unit** fassen (Auto-Restart, definierte Env), (2) Concurrency explizit setzen — `OLLAMA_NUM_PARALLEL` 2–4, `OLLAMA_MAX_LOADED_MODELS` 3–4 (35b-fast + 14b + bge-m3 warm), `OLLAMA_KEEP_ALIVE` lang/-1, ggf. ROCm-Env (`HSA_OVERRIDE_GFX_VERSION`), (3) `evo-health-monitor` reparieren. **NICHT mehrere Instanzen desselben Modells** — Parallelität via NUM_PARALLEL (Batching), Skalierung via Router-Spokes.
