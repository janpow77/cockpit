...
def test_schleifenschutz():
    eigene = li.eigene_adresse("100.99.159.80:7843", 7843)
    assert li.zeigt_auf_sich("http://100.99.159.80:7843", eigene)
    assert li.url_aus({"url": "http://100.99.159.80:7843"}, eigene) is None
    assert li.url_aus({"url": "http://100.102.132.11:7843"}, eigene) == "http://100.102.132.11:7843"
    assert li.zeigt_auf_sich("http://127.0.0.1:7843/", li.eigene_adresse(None, 7843))
